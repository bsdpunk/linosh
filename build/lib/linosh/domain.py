...
def list_domains(api_key):
    ...
    endpoint = "https://api.linode.com/?api_key=" + api_key + "&api_action=domain.list"
...
def domain_resource_list(api_key, domain_id, resource_id=0):
    headers = {'content-type': 'application/json'}
    if resource_id == 0:
        endpoint = "https://api.linode.com/?api_key=" + api_key + "&api_action=domain.resource.list&DomainID="+ domain_id
        r = requests.get(endpoint, headers=headers)
        json_data = json.loads(r.text) 
    else:
        endpoint = "https://api.linode.com/?api_key=" + api_key + "&api_action=domain.resource.list&DomainID="+ domain_id + "&ResourceID="+resource_id
        r = requests.get(endpoint, headers=headers)
        json_data = json.loads(r.text) 
...
def domain_resource_create(api_key, domain_id, kind, name = None,target = None):
    ...
    if name is None:
        ...
        endpoint = "https://api.linode.com/?api_key=" + api_key + "&api_action=domain.resource.create&DomainID=" +domain_id + "&Type=" + kind + "&Name=" + name +"&Target=" + target
